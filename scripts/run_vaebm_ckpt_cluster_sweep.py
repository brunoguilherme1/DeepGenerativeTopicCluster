#!/usr/bin/env python
"""Durable, resumable driver for the vaebm_ckpt x 26-dataset cluster
sweep (FutureLab) - see models/vaebm_ckpt.py's own docstring for what
this model is: the SAME fixed-alpha VAE-BM architecture as plain
"vaebm", but run at alpha=0 (embedding-branch-only fusion, mu_f =
mu_emb) with the GTE-large embedder and a very low learning rate, using
vaebm_poe's own per-epoch oracle-checkpoint-selection loop (keeps
whichever of up to 50 epochs has the best label-informed clustering
accuracy, not just the final epoch) - user-authorized ablation,
2026-09-18, testing whether this converges toward the SBERT-GTE+KMeans
baseline's own behavior.

Mirrors run_vaebm3_cluster_sweep.py's architecture exactly (isolated
...
per-combo timeout with skip-not-retry-on-timeout) - just a single-model
sweep instead of three.

Usage:
    python scripts/run_vaebm_ckpt_cluster_sweep.py                      # start a new run
    python scripts/run_vaebm_ckpt_cluster_sweep.py --run-dir results/vaebm_ckpt_cluster_20260918_120000  # resume
"""
...
MAX_ATTEMPTS = 1  # same policy as run_vaebm3_cluster_sweep.py - a timeout is not retried
RETRY_BACKOFF_SECONDS = 20
PER_COMBO_TIMEOUT_SECONDS = 1800  # 30 min external safety net; VAEBM_CKPT_MAX_FIT_SECONDS
                                   # (20min) is the internal early-stop, same margin as poe/dec
...
class Sweep:
    ...
    def run_one(self, model: str, dataset: str, progress_idx: int) -> None:
        ...
        env["VAEBM_EMBEDDER"] = "thenlper/gte-large"
        env["VAEBM_CKPT_ALPHA"] = "0.0"
        env["VAEBM_CKPT_LR"] = "1e-5"
                ...
                if proc.returncode != 0:
                    ...
                    if outcome is None:
                        ...
                        self.checkpoint[key] = {
                            ...
                            "alpha": 0.0, "lr": 1e-5, "embedder": "thenlper/gte-large",
                            "timestamp": now_iso(),
                        }
        ...
        self.checkpoint[key] = {"status": "error", "attempts": final_attempt, "error": last_error[:2000],
                                 "alpha": 0.0, "lr": 1e-5, "embedder": "thenlper/gte-large", "timestamp": now_iso()}
        self.save_checkpoint()
        self.write_progress()
    ...
    def run(self) -> None:
        ...
        self.log(f"SWEEP_START run_dir={self.run_dir} alpha=0.0 lr=1e-5 embedder=thenlper/gte-large")
        ...
        self.write_progress()
        self.log("SWEEP_LOOP_DONE all combinations attempted")
        completed, successful, failed = self.counts()
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_ckpt_cluster_{timestamp}"
