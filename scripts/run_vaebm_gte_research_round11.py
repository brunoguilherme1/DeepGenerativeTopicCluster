#!/usr/bin/env python
"""Round 11 (THIRD EMBEDDER CONFIRMATION) of the "beat HiCOT K=50 via
GTE-mimicking VAE-BM" autonomous research pass (2026-09-19,
user-authorized) - see docs/vaebm_mimic_gte_research_log.md.

Rounds 6 and 9 already showed: a different embedder (bge-large-en-v1.5)
doesn't help, and even the raw embedding+KMeans ceiling (gte-large)
can't beat the 3 still-blocked targets (20ng/agnews NMI, imdb Cv) -
strong evidence these are structural, not gte-large-specific. This round
adds a THIRD, independent embedder (`intfloat/e5-large-v2` - also
1024-dim, own built-in Normalize module verified via HF modules.json,
trained on yet another objective/data mixture) to make that conclusion
airtight: both raw embedding+KMeans AND VAE-BM+freqwords, on the 3
blocked hicot_* datasets only (google_news/search_snippets already
solved - not re-tested).
...
Usage:
    python scripts/run_vaebm_gte_research_round11.py
    python scripts/run_vaebm_gte_research_round11.py --run-dir results/vaebm_gte_r11_20260919_190000
"""
...
CONFIGS = {
    "P_sbert_kmeans_e5": {
        "model": "sbert_kmeans",
        "env": {},
        "extra_args": ["--sbert-embedder", "intfloat/e5-large-v2"],
    },
    "Q_vaebm_freqwords_e5": {
        "model": "vaebm",
        "env": {
            "VAEBM_EMBEDDER": "intfloat/e5-large-v2",
            "VAEBM_UNITS": "1024",
            "VAEBM_FREEZE_EMB": "1",
            "VAEBM_ALPHA": "0.0",
...
class Sweep:
    ...
    def run(self) -> None:
        self.log(f"SWEEP_START run_dir={self.run_dir} configs={list(CONFIGS.keys())}")
        idx = 0
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r11_{timestamp}"
