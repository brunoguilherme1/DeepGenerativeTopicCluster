#!/usr/bin/env python
"""Round 31 (CONFIRMATORY - does lambda<0.1 extend the trend further?)
of the systematic K=50 VAE-BM search (2026-09-20, user-authorized) - see
docs/vaebm_leaderboard.md's own "Round 31 (next, optional refinement)"
section.

Round 30 achieved the headline result of the entire pass: at
lambda=0.1, hicot_search_snippets, hicot_google_news, AND hicot_imdb
(+BGE) ALL simultaneously beat HiCOT's official K=50 Cv/Purity/NMI
targets - the first full 3/3 wins of the whole project, fully
unsupervised. The lambda trend was cleanly monotonic (lower lambda ->
higher Cv) across the entire 0.7->0.1 range tested with zero reversal.

This is NOT required to confirm the 3/3 wins - already achieved and
already reported. It is one quick confirmatory check: does lambda=0.05
extend the trend further (larger, more robust margins) or has it
started to turn? Informs whether 0.1 should be reported as
"near-optimal" or merely "good enough, could still improve."

Tests lambda=0.05 on the same 3 winning datasets. hicot_imdb uses
BGE-large (its own established best embedder from Round 21); the other
two keep gte-large.

3 combos x 2 cv_methods = 6 combos. Otherwise mirrors Rounds 17-30's
own architecture (isolated subprocess per combo, JSON checkpoint/
resume, run.log/current_status.txt/progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round31.py
    python scripts/run_vaebm_gte_research_round31.py --run-dir results/vaebm_gte_r31_20260920_190000
"""
...
COMBOS = [
    (f"U_lambda{lam.replace('.', '')}", dataset, {
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r31_{timestamp}"
