#!/usr/bin/env python
"""Classification experiment (K=n_classes, supervised, SINGLE seed=42) for
the locked VAE-BM architecture across the same 12 plain (non-hicot_*)
datasets as run_vaebm_classification_locked_12ds.py (2026-09-21,
...
Unlike that script (5 seeds, mean+/-std), this driver runs seed=42 ONLY,
matching the seed every baseline (LDA, GloCOM, ECRTM, S2WTM, FASTopic,
HiCOT, BERTopic) already uses for classification - so VAE-BM (ours) gets
a same-seed, same-split-generation-rule (random_state=seed) row directly
comparable to every baseline's own single-seed result, rather than only a
5-seed mean that never included seed 42 itself.

Supports an optional --datasets filter (comma-separated dataset ids) so
this can be split across multiple GPUs/processes; CUDA_VISIBLE_DEVICES is
inherited from the calling shell's environment, not set here.

Usage:
    python scripts/run_vaebm_classification_seed42_12ds.py
    python scripts/run_vaebm_classification_seed42_12ds.py --datasets 20ng,agnews_short,imdb
    python scripts/run_vaebm_classification_seed42_12ds.py --run-dir results/vaebm_classif_seed42_20260921_190000
"""
...
PER_COMBO_TIMEOUT_SECONDS = 1200  # 1 seed in one subprocess call
...
class Sweep:
    def __init__(self, run_dir: Path, datasets: list):
    ...
    def _read_per_seed_rows(self, dataset: str) -> list:
        ...
        # scripts/run_experiment.py's own _run_classification().
    ...
    def _read_aggregate(self, dataset: str):
    ...
    def run(self) -> None:
        self.log(f"SWEEP_START run_dir={self.run_dir} datasets={[d for d, _ in self.datasets]} seeds={SEEDS} base_env={BASE_ENV}")
        for idx, (dataset, overrides) in enumerate(self.datasets, start=1):
...
def main() -> None:
    ...
    parser.add_argument("--datasets", default=None,
                         help="comma-separated subset of dataset ids to run (default: all 12)")
    args = parser.parse_args()

    if args.datasets:
        wanted = set(args.datasets.split(","))
        datasets = [(d, o) for d, o in ALL_DATASETS if d in wanted]
        missing = wanted - {d for d, _ in datasets}
        if missing:
            raise SystemExit(f"unknown dataset id(s): {missing}")
    else:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_classif_seed42_{timestamp}"
        run_dir.mkdir(parents=True, exist_ok=False)

    Sweep(run_dir, datasets).run()
