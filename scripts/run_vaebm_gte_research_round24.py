#!/usr/bin/env python
"""Round 24 (RE-ENGAGE THE BoW FUSION BRANCH - alpha>0) of the
systematic K=50 VAE-BM search (2026-09-20, user-authorized) - see
docs/vaebm_leaderboard.md's own "Major finding" and "Round 24 (next)"
sections.

Checked models/vaebm.py::Encoder.call directly: `mu = alpha * mu_bow +
(1 - alpha) * mu_emb`. Every single round in this entire pass (1-23)
used VAEBM_ALPHA=0.0 - meaning mu = mu_emb EXACTLY, every time. The
BoW/TF-IDF branch has been computed but its contribution multiplied by
zero and discarded before clustering in literally every experiment run
so far. This whole pass has been testing a frozen-embedding-encoder-
plus-KMeans pipeline wearing a "VAE-BM" label, not the genuine
BoW+embedding fusion HiCOT/ECRTM's own architecture (and this project's
own alpha default of 0.99) was designed around.

Round 23 showed three different embedding families (GTE, BGE, E5) all
converge to Cv~0.395-0.399 for hicot_google_news and hicot_20ng -
embedder swapping is exhausted as a lever for these two. This tests
whether re-engaging the BoW branch moves them: VAEBM_ALPHA=0.5 (genuine
50/50 fusion, not HiCOT's own 0.99 default - a more conservative first
step to see if the BoW signal moves anything before going further) with
VAEBM_EPOCHS raised from 1 to 20 (the BoW branch's MLP is otherwise
near its random init at alpha=0's 1-epoch setting and has had no chance
to learn anything useful to contribute to the blend once alpha>0 makes
it matter). freeze_embedding_branch stays on - only the BoW branch and
fusion weights should train, isolating the effect of adding the BoW
signal rather than also changing how the embedding branch is trained.

Uses gte-large (each dataset's own established embedder - neither
google_news nor 20ng benefited from BGE/E5, so no reason to switch here).

Combo grain: (dataset, cv_method), 2 datasets x 2 cv_methods = 4 combos.
Otherwise mirrors Rounds 17-23's own architecture (isolated subprocess
per combo, JSON checkpoint/resume, run.log/current_status.txt/
progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round24.py
    python scripts/run_vaebm_gte_research_round24.py --run-dir results/vaebm_gte_r24_20260920_190000
"""
...
K = 50
SEED = 42
DATASETS = ["hicot_google_news", "hicot_20ng"]
...
def combo_key(dataset: str, cv_method: str) -> str:
    return f"K_alpha05_epochs20|{dataset}|{cv_method}"
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r24_{timestamp}"
