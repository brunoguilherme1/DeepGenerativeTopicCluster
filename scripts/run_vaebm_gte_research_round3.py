#!/usr/bin/env python
"""Round 3 of the "beat HiCOT K=50 via GTE-mimicking VAE-BM" autonomous
research pass (2026-09-19, user-authorized) - see
docs/vaebm_mimic_gte_research_log.md for the full context/target table
and Round 2's own findings this builds on.

Round 2 found: freeze_embedding_branch=True fixes Purity/NMI at their
epoch-0 value forever (KMeans always clusters the same frozen mu) - only
Cv (decoder-driven) can still change with more training under freeze,
with ZERO risk to Purity/NMI. Round 3 tests two independent ideas:
  - D_freeze_epochs15: more decoder training (epochs=15, lr=1e-3) under
    full freeze - safe Cv-only lever, targets search_snippets/imdb's
    weak Cv.
  - E_freeze_alpha002 / F_freeze_alpha002_epochs15: alpha=0.02 (instead
    of 0) even with the embedding branch frozen - re-enables mu_bow's
    gradient path (mu = alpha*mu_bow + (1-alpha)*mu_emb only zeroes
    mu_bow's contribution when alpha is exactly 0), letting a tiny BoW
    signal shape the final mu without giving up most of freeze's
    benefit - targets agnews's NMI, stuck under full freeze.

Combo grain is (config_name, dataset) - config holds fixed model="vaebm"
and K=50. Otherwise mirrors run_vaebm3_topic_sweep.py's architecture
(isolated subprocess per combo, JSON checkpoint/resume, run.log/
current_status.txt/progress.txt, 30-min per-combo timeout).

Usage:
    python scripts/run_vaebm_gte_research_round3.py
    python scripts/run_vaebm_gte_research_round3.py --run-dir results/vaebm_gte_r3_20260919_190000
"""
...
CONFIGS = {
    "D_freeze_epochs15": {"VAEBM_FREEZE_EMB": "1", "VAEBM_ALPHA": "0.0", "VAEBM_EPOCHS": "15", "VAEBM_LR": "1e-3"},
    "E_freeze_alpha002": {"VAEBM_FREEZE_EMB": "1", "VAEBM_ALPHA": "0.02", "VAEBM_EPOCHS": "1", "VAEBM_LR": "1e-4"},
    "F_freeze_alpha002_epochs15": {"VAEBM_FREEZE_EMB": "1", "VAEBM_ALPHA": "0.02", "VAEBM_EPOCHS": "15", "VAEBM_LR": "1e-3"},
}
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r3_{timestamp}"
