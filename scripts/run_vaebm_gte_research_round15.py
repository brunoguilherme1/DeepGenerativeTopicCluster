#!/usr/bin/env python
"""Round 15 ("TOPIC-WORD GENERATION" PASS, refined) of the "beat HiCOT
K=50 via GTE-mimicking VAE-BM" autonomous research pass (2026-09-20,
user-authorized) - see docs/vaebm_mimic_gte_research_log.md for the
full context/table and Round 14's own findings this builds on.

Round 14 found: HiCOT's own vocab.txt makes zero measurable difference
(vocabulary fidelity ruled out - VAE-BM's own independently-fit
vocabulary already converges to essentially the same word set). Its
GloVe-centroid topic-word re-ranking UNDERPERFORMED plain frequency
ranking on Palmetto C_V on all 5 datasets - suspected cause: re-ranking
over EVERY word present in a cluster (often hundreds) dilutes relevance.

This round tests the fix: `static_candidate_pool` (new
models/vaebm.py::top_words_by_freq_exact parameter, default 40 via
VAEBM_STATIC_CANDIDATE_POOL) narrows re-ranking to the top-N most
frequent present words first, THEN ranks by cosine similarity to that
pool's own frequency-weighted centroid in HiCOT's own GloVe space -
config V_hicot_static_glove_pool40. If this still underperforms plain
frequency ranking, the whole static-embedding-centroid direction should
be considered ruled out, not just this one implementation detail.
...
Usage:
    python scripts/run_vaebm_gte_research_round15.py
    python scripts/run_vaebm_gte_research_round15.py --run-dir results/vaebm_gte_r15_20260920_190000
"""
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r15_{timestamp}"
