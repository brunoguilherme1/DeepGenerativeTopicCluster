#!/usr/bin/env python
"""Round 8 (10-DATASET COVERAGE) of the "beat HiCOT K=50 via GTE-mimicking
VAE-BM" autonomous research pass (2026-09-19, user-authorized) - see
docs/vaebm_mimic_gte_research_log.md for the full context/table.

SCOPE GAP FOUND: the user's original request explicitly listed BOTH
variants per dataset ("hicot_20ng and 20ng and hicot_search_snippets and
search_snippets, etc") - 10 datasets total. Round 1's baseline sweep
covered all 10, but Rounds 2-7 (every architectural idea: freeze,
kl_weight, alpha variants, epochs, top_words_mode, vaebm_dec, units
truncation, different embedder, protocol validation) only ever tested
the 5 hicot_* variants. This round closes that gap: runs the final
validated best config (L_freqwords_correct_protocol from Round 7 -
freeze=1, alpha=0, epochs=1, lr=1e-4, top_words_mode="freq", units=1024,
dim_emb="", gte-large, --protocol ecrtm_hicot) on the 5 NON-hicot
counterpart datasets (20ng, search_snippets, google_news_ts,
agnews_short, imdb). Note: HiCOT's own reference table only reports
numbers for the hicot_* artifact/split - these 5 plain-dataset results
have no official HiCOT target to compare against; this round is about
confirming the recipe generalizes (or doesn't) beyond the 5 already
covered, not a beats/misses verdict.

Combo grain is (dataset), single config. Otherwise mirrors
...
Usage:
    python scripts/run_vaebm_gte_research_round8.py
    python scripts/run_vaebm_gte_research_round8.py --run-dir results/vaebm_gte_r8_20260919_190000
"""
...
CONFIGS = {
    "L_freqwords_correct_protocol": {"VAEBM_TOP_WORDS_MODE": "freq"},
}
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r8_{timestamp}"
