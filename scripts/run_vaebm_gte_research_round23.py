#!/usr/bin/env python
"""Round 23 (EMBEDDER SENSITIVITY FOR THE WORST REMAINING GAP) of the
systematic K=50 VAE-BM search (2026-09-20, user-authorized) - see
docs/vaebm_leaderboard.md's own "Round 23 (next)" section.

After Rounds 17-22, GoogleNews has the single worst Cv gap of any
hicot_* dataset (-0.056) and 20NG the second-worst (-0.052) - neither
has moved under freq-mode, GloVe-hybrid, normalize_mu, or BGE-large
(Round 21 already tested BGE on both: ~flat for both, no help). This
tests one more untried embedder, `intfloat/e5-large-v2`, on exactly
these two stubborn datasets.

Caveat, noted explicitly rather than silently glossed over: E5 models
are benchmarked WITH "query: "/"passage: " text prefixes for their
reported MTEB accuracy. This project's embedder.encode() call (models/
vaebm.py) adds no prefix, so this is a lower-bound/ablation test of E5's
raw capability, not its manufacturer-optimal configuration - unlike
BGE/GTE, which don't require prefixes for competitive performance per
their own model cards. Still a genuine, informative new embedding-family
data point either way (a positive result here would be even more
notable given the handicap; a negative result is not fully conclusive
about E5's true ceiling).

Recipe otherwise matches Round 17 (freq+stopword, no normalize_mu/
GloVe-hybrid - isolate the embedder swap's own effect, consistent with
how BGE was first tested in Round 21).

Combo grain: (dataset, cv_method), 2 datasets x 2 cv_methods = 4 combos.
Otherwise mirrors Rounds 17-22's own architecture (isolated subprocess
per combo, JSON checkpoint/resume, run.log/current_status.txt/
progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round23.py
    python scripts/run_vaebm_gte_research_round23.py --run-dir results/vaebm_gte_r23_20260920_190000
"""
...
def combo_key(dataset: str, cv_method: str) -> str:
    return f"J_e5_large|{dataset}|{cv_method}"
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r23_{timestamp}"
