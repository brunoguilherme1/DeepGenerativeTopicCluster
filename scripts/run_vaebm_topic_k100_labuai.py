#!/usr/bin/env python
"""labuai counterpart to run_vaebm_gte_research_round32_k100_imdb.py
(FutureLab): the other 8 of the 10-dataset K=100 topic sweep (2026-09-20,
user-authorized) - see docs/vaebm_leaderboard.md's "FINAL Stage D
...
IMDB pair is deliberately excluded here (runs on FutureLab instead,
per the compute-environment split agreed for this round - imdb-family
documents are this project's most memory-intensive and have repeatedly
OOM'd on labuai's weaker GPUs throughout this whole research pass).

Sequential (not the threaded multi-worker pattern of
run_vaebm3_topic_sweep_labuai.py) - deliberately simpler/more
predictable for this one-off run, matching this session's own
FutureLab round-script convention rather than reusing that script's own
MODELS=[vaebm,vaebm_poe,vaebm_dec]/fixed-gte-large-only design, which
does not support per-dataset lambda/normalize_mu overrides.

Usage:
    python scripts/run_vaebm_topic_k100_labuai.py
    python scripts/run_vaebm_topic_k100_labuai.py --run-dir results/vaebm_topic_k100_labuai_20260920_190000
"""
...
PYTHON_BIN = os.environ.get("VAEBM_PYTHON_BIN", "/ssd/bruno.gomes/venvs/dgtc/bin/python")
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_topic_k100_labuai_{timestamp}"
