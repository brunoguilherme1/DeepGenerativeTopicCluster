#!/usr/bin/env python
"""Round 32 (K=100, IMDB PAIR) of the systematic VAE-BM search
(2026-09-20, user-authorized) - first K=100 datapoint for the paper's
main topic-modeling table (docs/vaebm_leaderboard.md's "FINAL Stage D
status" / NAACL_HLT_2021_Latex_Template__1_/main.tex Table
tab:vaebm-topic-20ng-agnews-imdb-k100).

Every prior round (1-31) used K=50 only. This applies the exact locked
K=50 winning recipe for the IMDB pair (bge-large embedder, relevance-mode
topic words, lambda=0.05, no normalize_mu) at K=100, on FutureLab
specifically (imdb-family documents are the most memory-intensive in
this project's whole dataset registry - repeatedly OOM'd on labuai's
weaker GPUs throughout this session, per the compute-environment split
agreed for this round: labuai handles the other 8 K=100 combos,
FutureLab handles this one).

Usage:
    python scripts/run_vaebm_gte_research_round32_k100_imdb.py
    python scripts/run_vaebm_gte_research_round32_k100_imdb.py --run-dir results/vaebm_gte_r32_20260920_190000
"""
...
def combo_key(dataset: str, cv_method: str) -> str:
    return f"V_k100_imdb|{dataset}|{cv_method}"
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r32_{timestamp}"
