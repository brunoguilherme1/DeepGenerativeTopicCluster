#!/usr/bin/env python
"""Durable, resumable driver for the vaebm_ckpt x 10-dataset x 2-K topic
sweep (FutureLab) - see run_vaebm_ckpt_cluster_sweep.py's own docstring
for what this model is (alpha=0, GTE-large, very low LR, oracle-
checkpoint-selected epoch). Note: the topic experiment's own
run_single() never passes labels to fit() either (runner.py::run_single,
unchanged for every model) - epoch selection here falls back to
training-loss-based selection automatically, same as vaebm_poe/vaebm_dec's
own topic results.

Same 10-dataset HiCOT-pair list and (model, dataset, k) combo grain as
run_vaebm3_topic_sweep.py - mirrors its architecture exactly.

Usage:
    python scripts/run_vaebm_ckpt_topic_sweep.py
    python scripts/run_vaebm_ckpt_topic_sweep.py --run-dir results/vaebm_ckpt_topic_20260918_120000
"""
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_ckpt_topic_{timestamp}"
