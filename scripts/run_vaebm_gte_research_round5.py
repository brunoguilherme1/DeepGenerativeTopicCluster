#!/usr/bin/env python
"""Round 5 of the "beat HiCOT K=50 via GTE-mimicking VAE-BM" autonomous
research pass (2026-09-19, user-authorized) - see
docs/vaebm_mimic_gte_research_log.md for the full context/table and
Rounds 1-4's own findings this builds on.

Standing best after Round 4: G_freeze_freqwords (freeze=1, alpha=0,
epochs=1, lr=1e-4, top_words_mode="freq") - 12/15 beats, 2/5 datasets
(google_news, search_snippets) at full 3/3. Round 5 tests whether
VAEBM_UNITS < 1024 (a literal truncation to the first N dims of the raw
gte-large embedding, since mu_emb is frozen/identity-initialized - no
training involved either way) happens to separate the 3 still-blocked
datasets' classes better than the full 1024-dim space: hicot_20ng (NMI
stuck 0.003 short), hicot_agnews (NMI stuck ~0.371 vs 0.412 target
across every config tried), hicot_imdb (Cv stuck ~0.385 vs 0.404
target). google_news/search_snippets already at 3/3 - not re-tested.
...
Usage:
    python scripts/run_vaebm_gte_research_round5.py
    python scripts/run_vaebm_gte_research_round5.py --run-dir results/vaebm_gte_r5_20260919_190000
"""
...
CONFIGS = {
    "I_units768": {"VAEBM_UNITS": "768"},
    "J_units512": {"VAEBM_UNITS": "512"},
}
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r5_{timestamp}"
