#!/usr/bin/env python
"""Round 34 (K=100-SPECIFIC LAMBDA TUNING) of the systematic VAE-BM
search (2026-09-20, user-authorized) - see docs/vaebm_leaderboard.md's
own "Round 34 (next)" section.

Rounds 32/33 found that applying the K=50-tuned lambda_relevance value
unchanged at K=100 loses the full 3/3 win on hicot_search_snippets and
hicot_google_news (both miss Cv), while hicot_20ng/hicot_agnews stay
2/3 with an unmoved NMI gap. Since lambda was only ever swept at K=50
(Rounds 26-31), this tests whether a K=100-specific lambda recovers the
lost wins - mirroring the exact K=50 sweep methodology, just at K=100,
testing every lambda value NOT already covered by Round 33's own K=100
run for each dataset:
  - hicot_20ng, hicot_agnews (Round 33 already covered lambda=0.5):
    test {0.05, 0.1, 0.2, 0.3, 0.7}.
  - hicot_search_snippets, hicot_google_news (Round 33 already covered
    lambda=0.05): test {0.1, 0.2, 0.3, 0.5, 0.7}.
normalize_mu stays at its established per-dataset setting (on for
20ng/agnews, off for search_snippets/google_news) - only lambda varies.

5 lambdas x 4 datasets x 2 cv_methods = 40 combos. Otherwise mirrors
Rounds 17-33's own architecture (isolated subprocess per combo, JSON
checkpoint/resume, run.log/current_status.txt/progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round34_k100_lambda.py
    python scripts/run_vaebm_gte_research_round34_k100_lambda.py --run-dir results/vaebm_gte_r34_20260920_190000
"""
...
# Each entry: (dataset, base overrides (normalize_mu etc.), lambdas to test).
DATASET_PLANS = [
    ("hicot_20ng", {"VAEBM_NORMALIZE_MU": "1"}, ["0.05", "0.1", "0.2", "0.3", "0.7"]),
    ("hicot_agnews", {"VAEBM_NORMALIZE_MU": "1"}, ["0.05", "0.1", "0.2", "0.3", "0.7"]),
    ("hicot_search_snippets", {}, ["0.1", "0.2", "0.3", "0.5", "0.7"]),
    ("hicot_google_news", {}, ["0.1", "0.2", "0.3", "0.5", "0.7"]),
]

# Flattened: (label, dataset, env overrides).
COMBOS = [
    (f"X_lambda{lam.replace('.', '')}", dataset, {**base_overrides, "VAEBM_LAMBDA_RELEVANCE": lam})
    for dataset, base_overrides, lambdas in DATASET_PLANS
    for lam in lambdas
]
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r34_{timestamp}"
