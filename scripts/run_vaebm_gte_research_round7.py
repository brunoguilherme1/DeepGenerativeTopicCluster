#!/usr/bin/env python
"""Round 7 (PROTOCOL VALIDATION) of the "beat HiCOT K=50 via GTE-mimicking
...
CRITICAL METHODOLOGICAL FIX: Rounds 1-6 all ran under run_experiment.py's
DEFAULT "generic" protocol (top_n=10, local-corpus gensim Cv) - NONE of
them passed --protocol ecrtm_hicot, which is what actually aligns top_n
(15, not 10) with HiCOT's own paper convention (see run_experiment.py's
own --protocol help text). Every "beats/misses" verdict in Rounds 1-6 is
therefore NOT a strict apples-to-apples comparison against the user's
HiCOT K=50 reference table. This round re-validates the two strongest
candidates (A_freeze/energy-mode and G_freeze_freqwords) under the
CORRECT --protocol ecrtm_hicot, on all 5 target datasets, before drawing
any final conclusion. (Palmetto/Wikipedia-based Cv - HiCOT's own actual
Cv computation - is a further, heavier step not attempted here due to
its ~5.1GB one-time download; --cv-method still defaults to "local"
under ecrtm_hicot too, per --protocol's own documented behavior. This
remains a caveat on the final numbers either way.)
...
Usage:
    python scripts/run_vaebm_gte_research_round7.py
    python scripts/run_vaebm_gte_research_round7.py --run-dir results/vaebm_gte_r7_20260919_190000
"""
...
CONFIGS = {
    "M_energy_correct_protocol": {"VAEBM_TOP_WORDS_MODE": "energy"},
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r7_{timestamp}"
