#!/usr/bin/env python
"""Round 14 ("TOPIC-WORD GENERATION" PASS) of the "beat HiCOT K=50 via
GTE-mimicking VAE-BM" autonomous research pass (2026-09-20,
user-authorized) - see docs/vaebm_mimic_gte_research_log.md for the
full context/table and the FINAL CORRECTED CONCLUSION (Round 12/13)
this builds on.

New user directive: keep the proven clustering backbone (frozen
embedding branch, alpha=0, gte-large - the config with the best/closest
Purity/NMI found so far), and focus on IMPROVING TOPIC-WORD GENERATION
specifically, since Round 12/13 showed C_V (not Purity/NMI) is the real
remaining blocker under real Palmetto C_V - for BOTH VAE-BM and the raw
embedding+KMeans ceiling.

New idea this round (not tried before): HiCOT ships its own official
vocab.txt (5000/4618/3473 words, matching ECRTM's Table 9) and its own
200-dim GloVe word_embeddings.npz (matching ECRTM's Appendix B) for
exactly these 5 datasets - present in datasets/definitions/
hicot_datasets.py but never wired into experiment/runner.py before this
pass. Two configs, isolating each lever:
  - T_hicot_vocab_freq: VAEBM_USE_HICOT_VOCAB=1 only (keep top_words_mode
    "freq") - tests whether HiCOT's own fixed vocabulary (vs. VAE-BM's
    own independently-fit TfidfVectorizer vocabulary) changes C_V, with
    zero other change.
  - U_hicot_static_glove: VAEBM_USE_HICOT_VOCAB=1 + VAEBM_USE_HICOT_STATIC_EMB=1
    + VAEBM_TOP_WORDS_MODE=static - the main new idea: score topic words
    by cosine similarity to each cluster's own frequency-weighted
    centroid in HiCOT's own GloVe space (relevance-filtered to words
    that actually appear in the cluster, then coherence-ranked).

Per the user's explicit instruction, BOTH cv_local (gensim) and Palmetto
C_V are recorded for every combo - two subprocess invocations per
(config, dataset), same seed (deterministic - same trained model/
clusters/topic words either way, only which C_V function scores them
differs). Purity/NMI are read from whichever invocation completes
first (they don't depend on cv_method) and cross-checked identical on
the second for a cheap sanity check.

Combo grain is (config_name, dataset, cv_method). Otherwise mirrors
run_vaebm3_topic_sweep.py's architecture (isolated subprocess per combo,
JSON checkpoint/resume, run.log/current_status.txt/progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round14.py
    python scripts/run_vaebm_gte_research_round14.py --run-dir results/vaebm_gte_r14_20260920_190000
"""
...
PER_COMBO_TIMEOUT_SECONDS = 2400  # Palmetto's own JVM + Wikipedia index lookups are slower than local gensim Cv
...
class Sweep:
                ...
                if proc.returncode != 0:
                    ...
                    if outcome is None:
                        ...
                        self.checkpoint[key] = {
                            ...
                            "cv_method": cv_method, "config": CONFIGS[config_name], "timestamp": now_iso(),
                        }
    ...
    def run(self) -> None:
        self.log(f"SWEEP_START run_dir={self.run_dir} configs={list(CONFIGS.keys())} cv_methods={CV_METHODS}")
        idx = 0
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r14_{timestamp}"
