#!/usr/bin/env python
"""Round 6 of the "beat HiCOT K=50 via GTE-mimicking VAE-BM" autonomous
research pass (2026-09-19, user-authorized) - see
docs/vaebm_mimic_gte_research_log.md for the full context/table and
Rounds 1-5's own findings this builds on.

Round 5 found VAEBM_UNITS<1024 (dimensionality truncation of the frozen
embedding) never crosses any of the 3 remaining thresholds, and is
monotonically harmful for 20ng's NMI - ruled out. Round 6 tests the one
still-unexplored orthogonal axis: a DIFFERENT embedder. `BAAI/bge-large-
en-v1.5` is also 1024-dim (no units/dim_emb change needed) with a
built-in Normalize module (verified via its own modules.json on HF,
same as gte-large), trained on a different contrastive-retrieval mixture
- tests whether agnews/imdb/20ng's residual gaps are embedder-specific
(how well gte-large happens to separate these classes) rather than an
architectural limitation. Same G_freeze_freqwords recipe otherwise
(freeze=1, alpha=0, epochs=1, lr=1e-4, top_words_mode="freq", units=1024,
dim_emb="").

Combo grain is (dataset) - single config, only the embedder changes.
Otherwise mirrors run_vaebm3_topic_sweep.py's architecture (isolated
subprocess per combo, JSON checkpoint/resume, run.log/
current_status.txt/progress.txt, 30-min per-combo timeout).

Usage:
    python scripts/run_vaebm_gte_research_round6.py
    python scripts/run_vaebm_gte_research_round6.py --run-dir results/vaebm_gte_r6_20260919_190000
"""
...
CONFIGS = {
    "K_bge_large": {"VAEBM_EMBEDDER": "BAAI/bge-large-en-v1.5"},
}
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r6_{timestamp}"
