#!/usr/bin/env python
"""Round 29 (LAMBDA_RELEVANCE SWEEP for the remaining Cv-blocked
datasets) of the systematic K=50 VAE-BM search (2026-09-20,
user-authorized) - see docs/vaebm_leaderboard.md's own "Round 29 (next)"
section.

Round 26 found relevance mode (lambda=0.5, chosen as a reasonable
middle guess, never tuned) improves Cv on all 5 hicot_* datasets.
20NG and AGNews now beat their Cv target outright; hicot_search_snippets
(gap -0.010), hicot_google_news (gap -0.025), and hicot_imdb+BGE (gap
-0.025) remain short on Cv ONLY - every other metric on these 3 already
beats target. Since lambda was never tuned, this sweeps lambda in
{0.3, 0.7} (bracketing the untuned 0.5 default) on exactly these 3
datasets to see whether a better lambda pushes any of them over their
Cv target line. hicot_imdb uses BGE-large (its own established best
embedder from Round 21); the other two keep gte-large.

6 combos x 2 cv_methods = 12 combos. Otherwise mirrors Rounds 17-28's
own architecture (isolated subprocess per combo, JSON checkpoint/
resume, run.log/current_status.txt/progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round29.py
    python scripts/run_vaebm_gte_research_round29.py --run-dir results/vaebm_gte_r29_20260920_190000
"""
...
COMBOS = [
    (f"S_lambda{lam.replace('.', '')}", dataset, {
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r29_{timestamp}"
