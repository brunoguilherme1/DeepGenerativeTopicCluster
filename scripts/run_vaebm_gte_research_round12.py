#!/usr/bin/env python
"""Round 12 (REAL PALMETTO C_V VALIDATION) of the "beat HiCOT K=50 via
GTE-mimicking VAE-BM" autonomous research pass (2026-09-19,
user-authorized) - see docs/vaebm_mimic_gte_research_log.md.

Every prior round used `--cv-method local` (gensim, local training
corpus) - the user's own stated premise ("GTE-large+KMeans can exceed
many of these numbers") disagrees with Round 9's local-Cv finding that
raw embedding+KMeans MISSES several targets. The most likely explanation:
HiCOT's own reported C_V is Palmetto/Wikipedia-based, not local-corpus -
a fundamentally different (and typically higher, since it draws on a
much larger reference corpus) coherence signal. Palmetto is ALREADY
installed on FutureLab (tools/palmetto/palmetto.jar + wiki_data, ~6.5GB,
verified present - no download needed), so this round re-runs BOTH the
raw ceiling (sbert_kmeans+gte-large) and the winning VAE-BM recipe
(frozen+freqwords+gte-large) on all 5 hicot_* datasets with
`--cv-method palmetto`, to get the TRUE, HiCOT-matching C_V numbers.
Purity/NMI are unaffected by cv_method (already confirmed in Round 7) -
only C_V changes here.
...
Usage:
    python scripts/run_vaebm_gte_research_round12.py
    python scripts/run_vaebm_gte_research_round12.py --run-dir results/vaebm_gte_r12_20260919_190000
"""
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r12_{timestamp}"
