#!/usr/bin/env python
"""Round 28 (CAN KMEANS SEED/N_INIT CLOSE 20NG'S 0.0008 NMI GAP?) of the
systematic K=50 VAE-BM search (2026-09-20, user-authorized) - see
docs/vaebm_leaderboard.md's own "Round 28 (next, revised)" section.

Round 27 found relevance+normalize_mu on hicot_20ng lands 0.0008 short
of a full 3/3 win on NMI alone (Cv and Purity both comfortably beat
target). Before assuming seed variance could close this, checked
models/vaebm.py directly: with freeze_embedding_branch=1, dim_emb=(),
units=1024 (matching gte-large's own output width), and alpha=0, mu_emb's
Dense layer uses a DETERMINISTIC Identity(gain=0.99999) kernel
initializer (not random), and Encoder.__init__ hardcodes
tf.random.set_seed(1234) regardless of the run's own --seed. So `--seed`
has ZERO effect on this recipe's output - a --seed sweep would be a
wasted round. The ONLY genuine remaining source of stochasticity is
KMeans's own centroid initialization, fixed at random_state=22 by
default.

Tests VAEBM_KMEANS_SEED in {1, 7, 13, 99} (22 is already the existing
Round 27 result, not re-run) and VAEBM_KMEANS_N_INIT=20 (more restarts,
best-inertia selection - itself legitimate and label-free) on
hicot_20ng's exact winning recipe (relevance+normalize_mu). Palmetto Cv
only (the decision-relevant metric, and NMI/Purity/TD all come from the
SAME evaluate() call regardless of which cv_method was requested, so a
single palmetto-mode run already reports every metric needed to check
for a full 3/3).

5 combos, all on hicot_20ng. Otherwise mirrors Rounds 17-27's own
architecture (isolated subprocess per combo, JSON checkpoint/resume,
run.log/current_status.txt/progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round28.py
    python scripts/run_vaebm_gte_research_round28.py --run-dir results/vaebm_gte_r28_20260920_190000
"""
...
# Each entry: (label, env overrides layered on top of BASE_ENV).
COMBOS = [
    ("Q_kmeans_seed1", {"VAEBM_KMEANS_SEED": "1"}),
    ("Q_kmeans_seed7", {"VAEBM_KMEANS_SEED": "7"}),
    ("Q_kmeans_seed13", {"VAEBM_KMEANS_SEED": "13"}),
    ("Q_kmeans_seed99", {"VAEBM_KMEANS_SEED": "99"}),
    ("R_kmeans_ninit20", {"VAEBM_KMEANS_N_INIT": "20"}),
]
...
def combo_key(label: str) -> str:
    return f"{label}|{DATASET}|palmetto"
...
class Sweep:
    ...
    def write_status(self, label: str, attempt: int) -> None:
        completed, successful, failed = self.counts()
        text = (
    ...
    def run_one(self, label: str, overrides: dict, progress_idx: int) -> None:
        key = combo_key(label)
        existing = self.checkpoint.get(key)
        if existing and existing.get("status") == "ok":
            self.log(f"SKIP label={label} progress={progress_idx}/{self.total} "
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            self.write_status(label, attempt)
            self.log(f"START label={label} attempt={attempt} progress={progress_idx}/{self.total} overrides={overrides}")
            ...
            try:
                ...
                if proc.returncode != 0:
                    ...
                    if outcome is None:
                        ...
                        self.write_progress()
                        self.log(f"OK label={label} runtime={runtime:.0f}s "
            ...
            self.log(f"ERROR label={label} attempt={attempt} error={tag}")

            if timed_out:
                final_attempt = attempt
                break
            if attempt < MAX_ATTEMPTS:
                self.log(f"RETRY label={label} attempt={attempt + 1}")
        ...
        self.save_checkpoint()
        self.write_progress()
        self.log(f"ERROR label={label} FINAL after {final_attempt} attempt(s), giving up progress={progress_idx}/{self.total}")
    ...
    def run(self) -> None:
        self.log(f"SWEEP_START run_dir={self.run_dir} dataset={DATASET} combos={[c[0] for c in COMBOS]} base_env={BASE_ENV}")
        for idx, (label, overrides) in enumerate(COMBOS, start=1):
            self.run_one(label, overrides, idx)
        self.write_progress()
        completed, successful, failed = self.counts()
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r28_{timestamp}"
