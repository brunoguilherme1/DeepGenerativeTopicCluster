#!/usr/bin/env python
"""Round 35 (GOOGLENEWS K=100 LOW-LAMBDA CHECK) of the systematic
VAE-BM search (2026-09-20, user-authorized) - see
docs/vaebm_leaderboard.md's own "Round 35 (next, optional)" section.

Round 34 swept lambda in {0.1,0.2,0.3,0.5,0.7} at K=100 for
hicot_google_news and found none beat the already-known lambda=0.05
(Round 33) - a plateau, unlike hicot_20ng/hicot_agnews which both found
a better K=100-specific lambda in that same range. This mirrors the
K=50 pattern exactly: GoogleNews kept improving as lambda dropped below
0.1 with no sign of reversal all the way to the lowest value tested
there (0.05, Round 31). This tests lambda in {0.01, 0.02, 0.03} at
K=100 - the next lower values, continuing that same trend - before
concluding GoogleNews's K=100 Cv gap (-0.024) needs a different kind of
fix.

3 lambdas x 1 dataset x 2 cv_methods = 6 combos. Otherwise mirrors
Rounds 17-34's own architecture (isolated subprocess per combo, JSON
checkpoint/resume, run.log/current_status.txt/progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round35_k100_gnews_lowlambda.py
    python scripts/run_vaebm_gte_research_round35_k100_gnews_lowlambda.py --run-dir results/vaebm_gte_r35_20260920_190000
"""
...
# Each entry: (dataset, base overrides, lambdas to test).
DATASET_PLANS = [
    ("hicot_google_news", {}, ["0.01", "0.02", "0.03"]),
]

# Flattened: (label, dataset, env overrides).
COMBOS = [
    (f"Y_lambda{lam.replace('.', '')}", dataset, {**base_overrides, "VAEBM_LAMBDA_RELEVANCE": lam})
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r35_{timestamp}"
