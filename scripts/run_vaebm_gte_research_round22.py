#!/usr/bin/env python
"""Round 22 (DOES normalize_mu COMPOUND WITH BGE FOR IMDB?) of the
systematic K=50 VAE-BM search (2026-09-20, user-authorized) - see
docs/vaebm_leaderboard.md's own "Round 22 (next)" section.

Round 21 found BAAI/bge-large-en-v1.5 meaningfully improves hicot_imdb
on all 3 metrics vs gte-large (Cv 0.350 -> 0.362, Purity 0.803 -> 0.840,
NMI 0.115 -> 0.138) - the best hicot_imdb result of the whole pass, and
the first case this whole pass where a learned VAE-BM representation
exceeds its own raw sbert_kmeans+KMeans ceiling (0.362 vs 0.353). BGE
was flat-to-worse on the other 4 hicot_* datasets, so it's adopted as
IMDB's own standing-best embedder only, not a universal swap.

Separately, Round 18's Finding D showed VAEBM_NORMALIZE_MU=1 gives
small, Cv-safe Purity/NMI gains on gte-large for 20NG/AGNews - but
Round 19 showed it does NOT always compound cleanly (stacking it onto
search_snippets' GloVe-hybrid config traded Cv away). normalize_mu has
never been tried with BGE at all. This round tests the one clean
combination: BGE (Round 21's IMDB winner) + normalize_mu (an orthogonal
clustering-geometry knob, not another topic-word-extraction change like
GloVe-hybrid was) on hicot_imdb specifically, single-variable per
Round 19's lesson about not stacking multiple untested ideas at once.

2 combos (local + Palmetto Cv). Otherwise mirrors Rounds 17-21's own
architecture (isolated subprocess per combo, JSON checkpoint/resume,
run.log/current_status.txt/progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round22.py
    python scripts/run_vaebm_gte_research_round22.py --run-dir results/vaebm_gte_r22_20260920_190000
"""
...
def combo_key(cv_method: str) -> str:
    return f"I_bge_normalize_mu|{DATASET}|{cv_method}"
...
class Sweep:
    ...
    def write_status(self, cv_method: str, attempt: int) -> None:
        completed, successful, failed = self.counts()
        text = (
            f"Dataset: {DATASET}\nCV method: {cv_method}\nAttempt: {attempt}\n"
    ...
    def run_one(self, cv_method: str, progress_idx: int) -> None:
        key = combo_key(cv_method)
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            self.write_status(cv_method, attempt)
    ...
    def run(self) -> None:
        self.log(f"SWEEP_START run_dir={self.run_dir} dataset={DATASET} cv_methods={CV_METHODS} base_env={BASE_ENV}")
        idx = 0
        for cv_method in CV_METHODS:
            idx += 1
            self.run_one(cv_method, idx)
        self.write_progress()
        completed, successful, failed = self.counts()
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r22_{timestamp}"
