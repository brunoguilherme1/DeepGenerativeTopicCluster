#!/usr/bin/env python
"""Round 4 of the "beat HiCOT K=50 via GTE-mimicking VAE-BM" autonomous
research pass (2026-09-19, user-authorized) - see
docs/vaebm_mimic_gte_research_log.md for the full context/target table
and Rounds 2-3's own findings this builds on.

Round 3 found alpha>0 (even 0.02) under freeze is a net negative
(destabilizes clustering) - abandoned. Standing best is A_freeze/
D_freeze_epochs15, tied at 11/15 beats, blocked on: search_snippets/imdb
Cv (decoder/topic-word quality), agnews NMI (clustering geometry, frozen
mu can't be reshaped by decoder-side changes).

Round 4 tests two NEW, unrelated ideas:
  - G_freeze_freqwords: same as A_freeze but top_words_mode="freq"
    (per-cluster word-frequency counts) instead of "energy" (decoder
    R-matrix-driven) - tests whether Cv was a decoder-quality problem or
    a topic-word-extraction-method problem. Zero risk to Purity/NMI
    (doesn't touch mu). model=vaebm.
  - H_vaebm_dec: model="vaebm_dec" (untested in this research pass so
    far) - Deep Embedded Clustering trained JOINTLY via its own
    clustering loss (lambda_c=0.1), NOT frozen (freeze=0), since DEC's
    whole point is a clustering-aware training signal (unlike the BoW
    branch's reconstruction objective, which has no interest in cluster
    quality) - targets agnews's NMI ceiling and 20ng's near-miss.

Combo grain is (config_name, dataset) - each config specifies its own
model + env overrides. Otherwise mirrors run_vaebm3_topic_sweep.py's
...
Usage:
    python scripts/run_vaebm_gte_research_round4.py
    python scripts/run_vaebm_gte_research_round4.py --run-dir results/vaebm_gte_r4_20260919_190000
"""
...
CONFIGS = {
    "G_freeze_freqwords": {
        "model": "vaebm",
        "env": {"VAEBM_FREEZE_EMB": "1", "VAEBM_ALPHA": "0.0", "VAEBM_EPOCHS": "1", "VAEBM_LR": "1e-4",
                "VAEBM_TOP_WORDS_MODE": "freq"},
    },
    "H_vaebm_dec": {
        "model": "vaebm_dec",
        "env": {"VAEBM_FREEZE_EMB": "0", "VAEBM_ALPHA": "0.0", "VAEBM_EPOCHS": "15", "VAEBM_LR": "1e-3"},
    },
}
...
class Sweep:
    ...
    def run_one(self, config_name: str, dataset: str, progress_idx: int) -> None:
        ...
        model = CONFIGS[config_name]["model"]
        env.update(BASE_ENV)
        env.update(CONFIGS[config_name]["env"])
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r4_{timestamp}"
