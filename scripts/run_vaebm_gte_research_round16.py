#!/usr/bin/env python
"""Round 16 ("TOPIC-WORD GENERATION" PASS, soft blend) of the "beat
HiCOT K=50 via GTE-mimicking VAE-BM" autonomous research pass
(2026-09-20, user-authorized) - see docs/vaebm_mimic_gte_research_log.md
for the full context/table and Round 15's own findings this builds on.

Round 15 found the hard candidate-pool cutoff (top-40 most frequent
present words, then rank by similarity alone) is non-monotonic across
datasets: helped search_snippets/agnews meaningfully, HURT 20ng (worse
than even no pool restriction at all), roughly neutral elsewhere. Also
found local C_V and Palmetto C_V can move in OPPOSITE directions for
this technique - local C_V is not a reliable proxy here.

This round tests a SOFT blend instead: `static_hybrid_weight` (new
models/vaebm.py::top_words_by_freq_exact parameter) rank-normalizes
frequency and similarity to [0,1] over the full present-word candidate
set (no hard pool cutoff - VAEBM_STATIC_CANDIDATE_POOL left unset) and
combines them as `freq_rank + weight * sim_rank`. Two weights tested
(0.5: frequency-dominant, similarity as a tiebreaker-ish nudge; 1.5:
similarity-dominant) to get a first read on sensitivity before tuning
further.
...
Usage:
    python scripts/run_vaebm_gte_research_round16.py
    python scripts/run_vaebm_gte_research_round16.py --run-dir results/vaebm_gte_r16_20260920_190000
"""
...
CONFIGS = {
    "W_hicot_static_hybrid05": {
        "VAEBM_USE_HICOT_VOCAB": "1",
        "VAEBM_USE_HICOT_STATIC_EMB": "1",
        "VAEBM_TOP_WORDS_MODE": "static",
        "VAEBM_STATIC_CANDIDATE_POOL": "",
        "VAEBM_STATIC_HYBRID_WEIGHT": "0.5",
    },
    "X_hicot_static_hybrid15": {
        "VAEBM_USE_HICOT_VOCAB": "1",
        "VAEBM_USE_HICOT_STATIC_EMB": "1",
        "VAEBM_TOP_WORDS_MODE": "static",
        "VAEBM_STATIC_CANDIDATE_POOL": "",
        "VAEBM_STATIC_HYBRID_WEIGHT": "1.5",
    },
}
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r16_{timestamp}"
