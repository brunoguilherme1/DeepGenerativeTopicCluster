...
downstream classification sweep (models: vaebm, vaebm_poe, vaebm_dec -
see models/vaebm_poe.py, models/vaebm_dec.py) -
`--experiment classification --split random` (see
experiment/classification_runner.py's own module docstring): document ->
model representation (latent mu, for all three of these) -> SVM -> labels,
over a stratified 80/20 split we draw ourselves, K auto-derived per
dataset from its own num_classes (--k omitted). Default embedder
...
Note: unlike the cluster experiment (see run_vaebm3_cluster_sweep.py),
vaebm_poe/vaebm_dec do NOT get oracle-label-informed checkpoint selection
here - classification has a genuine held-out test split, so using test
labels to pick the best training epoch would be real leakage, not the
cluster experiment's more benign "which epoch of a transductive
unsupervised fit looks best" convenience (see
experiment/classification_runner.py's own module docstring and this
session's methodological notes). Both models fall back to loss-based
epoch selection here instead.
...
per-combo timeout with skip-not-retry-on-timeout) - adapted for the
classification experiment's own result shape (accuracy/f1 instead of
acc/nmi/purity).

Usage:
    python scripts/run_vaebm3_classification_sweep.py                      # start a new run
    python scripts/run_vaebm3_classification_sweep.py --run-dir results/vaebm3_classification_20260916_120000  # resume
"""
...
# own: one attempt only, uniform 30-minute cap for every model. vaebm_poe/
# vaebm_dec each have their own internal max_fit_seconds early-stop (see
# experiment/classification_runner.py::_build_model_for_classification),
# which keeps this subprocess-level timeout a pure safety net, not their
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm3_classification_{timestamp}"
