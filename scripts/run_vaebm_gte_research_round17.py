#!/usr/bin/env python
"""Round 17 (STAGE A RE-BASELINE WITH STOPWORD FIX) of the systematic
K=50 VAE-BM search (2026-09-20, user-authorized) - see
docs/vaebm_leaderboard.md and docs/vaebm_mimic_gte_research_log.md.

CRITICAL prerequisite finding: a diagnostic investigation found plain
`imdb`'s apparent Cv "win" over HiCOT (0.469 vs 0.404 target) was a
measurement artifact - its default vectorizer has no stopword filter, so
topic words were dominated by function words ("the", "and") and an
unstripped "<br />" HTML artifact, which score artificially high on
Palmetto C_V. This is suspected to affect every plain-dataset result in
this research pass. VAEBM_EXCLUDE_STOPWORDS (new this pass) fixes this
for both VAE-BM and the raw embedding+KMeans ceiling.

This round re-establishes Stage A (confirmed current-best values for
ALL 10 datasets) under the corrected measurement, for BOTH:
  - vaebm (the established best recipe: frozen embedding branch,
    alpha=0, gte-large, freq-based topic words)
  - sbert_kmeans (the raw embedding+KMeans ceiling, same embedder)
with VAEBM_EXCLUDE_STOPWORDS=1 (sklearn's own English stopword list).

Also corrects a second diagnostic finding: hicot_google_news is
TITLE-ONLY (avg 5.75 tokens), while `google_news_ts` (title+snippet,
avg 27.95 tokens) is a different, much richer document - 0 exact-text
overlap. `google_news_t` (title-only, 5,842/11,019 docs overlap with
hicot_google_news) is the correct plain-variant comparison and replaces
google_news_ts in this and all future rounds.

Combo grain is (model, dataset, cv_method). Both cv_local (gensim) and
Palmetto C_V are recorded per the user's explicit instruction. Otherwise
mirrors run_vaebm3_topic_sweep.py's architecture (isolated subprocess
per combo, JSON checkpoint/resume, run.log/current_status.txt/
progress.txt).

Usage:
    python scripts/run_vaebm_gte_research_round17.py
    python scripts/run_vaebm_gte_research_round17.py --run-dir results/vaebm_gte_r17_20260920_190000
"""
...
DATASETS = [
    ...
    "hicot_google_news", "google_news_t",
...
class Sweep:
    ...
    def run_one(self, model: str, dataset: str, cv_method: str, progress_idx: int) -> None:
        ...
        if model == "sbert_kmeans":
            cmd += ["--sbert-embedder", "thenlper/gte-large"]
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r17_{timestamp}"
