...
def _run_topic(args) -> None:
    ...
    # `--protocol` (which still controls top_n/TD). Palmetto is a
    # ~5.1GB one-time download (scripts/setup_palmetto.py) - it is NEVER
    # triggered unless `--cv-method palmetto` is EXPLICITLY passed.
    # Nothing passed (for EITHER protocol, including ecrtm_hicot) always
    # means local-corpus gensim C_V, no auto-install, no large download.
    effective_cv_method = args.cv_method if args.cv_method == "palmetto" else "local"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Topic experiment only: 'generic' (default) reproduces this runner's original metric computation "
             ...
             "computation with ECRTM (Wu et al., ICML 2023) and HiCOT (2025) as closely as possible: top_n=15, TD "
             "via the fixed-K*15-denominator Dieng definition - C_V still defaults to the local-corpus gensim "
             "number under EITHER protocol (see --cv-method) unless Palmetto is explicitly requested; this flag "
             "alone never triggers the ~5.1GB Palmetto/Wikipedia download. Datasets and preprocessing are NOT "
             "changed by this flag - this is metric-level alignment only, not a claim of exact reproduction. See "
             "experiment/runner.py's own docstring and docs/methodological_notes.md #10.",
    )
    parser.add_argument(
        ...
        help="Topic experiment only: overrides WHICH function computes C_V, independently of --protocol (which "
             "still controls top_n/TD). Default (nothing passed, for EITHER --protocol including ecrtm_hicot): "
             "'local' - local-corpus gensim C_V, no download, no Palmetto ever triggered unless you ask for it "
             "here explicitly. Pass 'palmetto' explicitly (under either --protocol) to get real Wikipedia C_V - "
             "only then does this install Palmetto/the Wikipedia index automatically if not already present "
             "(scripts/setup_palmetto.py::ensure_palmetto_ready, a ~5.1GB one-time download).",
    )
    parser.add_argument(
