#!/usr/bin/env python
"""Round 2 of the "beat HiCOT K=50 via GTE-mimicking VAE-BM" autonomous
research pass (2026-09-19, user-authorized) - see
docs/vaebm_mimic_gte_research_log.md for the full context/target table.

Isolates the two new knobs (VAEBM_KL_WEIGHT, VAEBM_FREEZE_EMB - see
models/vaebm.py::VAEBM's own comment) against the Round 1 "mimic-GTE"
baseline (gte-large, units=1024, dim_emb="", alpha=0, lr=1e-4, epochs=1),
on model=vaebm only (cleanest/fastest case), K=50 only, across the 5
hicot_* target datasets.
...
Usage:
    python scripts/run_vaebm_gte_research_round2.py
    python scripts/run_vaebm_gte_research_round2.py --run-dir results/vaebm_gte_r2_20260919_190000
"""
...
CONFIGS = {
    "A_freeze": {"VAEBM_FREEZE_EMB": "1", "VAEBM_KL_WEIGHT": "1.0"},
    "B_lowkl": {"VAEBM_FREEZE_EMB": "0", "VAEBM_KL_WEIGHT": "0.1"},
    "C_freeze_lowkl": {"VAEBM_FREEZE_EMB": "1", "VAEBM_KL_WEIGHT": "0.1"},
}
...
def main() -> None:
    ...
    if args.run_dir:
        ...
        run_dir = REPO_ROOT / "results" / f"vaebm_gte_r2_{timestamp}"
