...
\midrule
\textbf{VAE-BM (ours)} & \textbf{0.450} & \textbf{0.691} & 0.546 & 0.782 & \textbf{0.456} & \textbf{0.875} & 0.342 & 0.911 & \textbf{0.402} & \textbf{0.836} & \textbf{0.119} & 0.716 \\
\bottomrule
\end{tabular}
\vspace{2pt}
\caption{100-topic results, same layout and provenance as Table~\ref{tab:vaebm-topic-20ng-agnews-imdb-k50}, with $\lambda$ re-tuned for $K=100$ specifically ($\lambda=0.2$ for 20NG, $0.3$ for AGNews, vs.\ $0.5$ at $K=50$ -- \S\ref{sec:topics}) -- \texttt{normalize\_mu} unchanged. VAE-BM now beats HiCOT on $C_V$ and Purity on both 20NG and AGNews; NMI is unmoved by $\lambda$ by construction (Equation~\ref{eq:relevance} never touches clustering) and remains short on both (20NG: 0.546 vs.\ 0.568; AGNews: 0.342 vs.\ 0.388). IMDB alone holds a full 3-of-3 win at both $K$ values.}
