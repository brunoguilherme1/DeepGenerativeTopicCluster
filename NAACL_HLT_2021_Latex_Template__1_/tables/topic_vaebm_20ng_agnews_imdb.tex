...
\midrule
\textbf{VAE-BM (ours)} & 0.443 & \textbf{0.691} & 0.546 & 0.734 & \textbf{0.447} & \textbf{0.875} & 0.342 & 0.860 & \textbf{0.402} & \textbf{0.836} & \textbf{0.119} & 0.716 \\
\bottomrule
\end{tabular}
\vspace{2pt}
\caption{100-topic results, same layout and provenance as Table~\ref{tab:vaebm-topic-20ng-agnews-imdb-k50}, using the SAME $\lambda$/\texttt{normalize\_mu} values tuned for $K=50$ (not re-tuned for $K=100$ -- \S\ref{sec:limitations}). VAE-BM beats HiCOT's Purity on all three and $C_V$ on 20NG/AGNews, but NMI regresses below target on 20NG/AGNews at this $K$ (a $K=50$-tuned recipe applied unchanged, not a $K=100$-specific search); IMDB alone holds a full 3-of-3 win at both $K$ values.}
