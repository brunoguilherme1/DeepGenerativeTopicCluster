...
\midrule
\textbf{VAE-BM (ours)} & 0.449 & \textbf{0.864} & 0.471 & 0.972 & 0.446 & \textbf{0.801} & \textbf{0.878} & 0.973 \\
\bottomrule
\end{tabular}
\vspace{2pt}
\caption{100-topic results, same layout and provenance as Table~\ref{tab:vaebm-topic-ss-gn-k50}, with $\lambda$ re-tuned for $K=100$ specifically ($\lambda=0.1$ for SearchSnippets, vs.\ $0.05$ at $K=50$; $\lambda=0.05$ remained optimal for GoogleNews among $\{0.1,...,0.7\}$ tested -- \S\ref{sec:topics}). Re-tuning shrinks SearchSnippets's $C_V$ gap to essentially zero ($-0.0002$, from $-0.001$ at the untuned $K=50$ value) but does not close it; its NMI gap (0.471 vs.\ 0.480) is unmoved by $\lambda$ by construction. GoogleNews's $C_V$ gap (0.446 vs.\ 0.470) did not improve within the range searched -- lower values, mirroring the pattern that continued improving GoogleNews's $K=50$ result below $\lambda=0.1$ (\S\ref{sec:topics}), remain untried at $K=100$. Both still beat Purity comfortably, and GoogleNews's NMI still beats target.}
